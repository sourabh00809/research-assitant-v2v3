  ...
  return (
    <SidebarLayout>
      <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        <p className="mt-1 text-sm text-slate-600">Manage autonomous research agents and approvals.</p>

        {message && (
          <div className="mt-4 rounded-md border border-slate-200 bg-white p-3 text-sm font-semibold text-slate-600">{message}</div>
        )}

        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1fr]">
          <Panel title="Agent Controls" meta={`${agentRuns.length} runs`}>
            <div className="mb-4 flex flex-wrap gap-2">
              <button className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold disabled:opacity-50"
              ...
              <button className="rounded-md bg-emerald-700 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"
            ...
            {active?.execution_artifacts?.length ? (
              <div className="grid gap-3 md:grid-cols-2">
                {active.execution_artifacts.map((a) => (
                  <article key={a.id} className="rounded-md border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-center justify-between gap-2">
                      <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold uppercase text-emerald-800">{a.kind}</span>
                      <span className="text-xs text-slate-500">{a.run_id}</span>
                    </div>
                    <p className="mt-2 line-clamp-3 text-sm text-slate-700">{a.content || a.path || "Stored artifact"}</p>
                  </article>
                ))}
              </div>
