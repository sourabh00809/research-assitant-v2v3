...
export default function MemoryPage() {
  ...
  if (error) return <SidebarLayout><div className="mx-auto max-w-5xl px-4 py-6"><div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-800 dark:bg-red-900/30 dark:text-red-300">Failed to load memory.</div></div></SidebarLayout>;

  return (
    <SidebarLayout>
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Research Memory</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{project?.name} · {memory.length} saved items</p>
        </header>
              ...
              <article key={item.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-600 dark:bg-slate-800">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  {item.tags?.map((tag: string) => (
                    <span key={tag} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600 dark:bg-slate-700/50 dark:text-slate-300">{tag}</span>
                  ))}
                </div>
                <p className="text-sm text-slate-800 dark:text-slate-200">{item.content}</p>
                {item.source_ids?.length > 0 && (
                  <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">Sources: {item.source_ids.join(", ")}</p>
                )}
              </article>
            ))}
          </div>
