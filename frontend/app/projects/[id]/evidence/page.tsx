    ...
    <SidebarLayout>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-slate-900">Evidence Inspector</h1>
          <p className="mt-1 text-sm text-slate-500">{project?.name} · {allEvidence.length} total evidence items</p>
        </header>

        <div className="mb-4 flex flex-wrap items-center gap-3">
          <input
            className="w-64 rounded-md border border-slate-300 p-2 text-sm focus:border-emerald-500 focus:outline-none"
          ...
          <span className="text-xs text-slate-400">{filtered.length} of {allEvidence.length} shown</span>
        </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((item) => (
              <article key={item.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <p className="mb-3 text-sm leading-6 text-slate-800">{item.claim}</p>
                <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-2 text-xs text-slate-400">
                ...
                {item.source_quote && (
                  <details className="mt-2">
                    <summary className="cursor-pointer text-xs font-semibold text-emerald-600">Source Quote</summary>
                    <p className="mt-1 rounded-md bg-slate-50 p-2 text-xs text-slate-600 italic">{item.source_quote}</p>
                  </details>
                )}
              </article>
            ))}
