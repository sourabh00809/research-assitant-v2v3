    ...
    <SidebarLayout>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Experiment Planner</h1>
            <p className="mt-1 text-sm text-slate-500">{project?.name} · {plans.length} experiment plans</p>
          </div>
          <button
            ...
            {plans.map((plan) => (
              <Panel key={plan.id} title={plan.title || plan.objective || "Experiment Plan"} meta={plan.status}>
                <div className="grid gap-3 text-sm">
                  {plan.objective && <p className="text-slate-600">{plan.objective}</p>}

                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {(plan.datasets?.length ?? 0) > 0 && (
                      <div>
                        <span className="font-semibold text-slate-700">Datasets:</span>
                        <p className="text-slate-500">{plan.datasets!.join(", ")}</p>
                      </div>
                    )}
                    {(plan.baselines?.length ?? 0) > 0 && (
                      <div>
                        <span className="font-semibold text-slate-700">Baselines:</span>
                        <p className="text-slate-500">{plan.baselines!.join(", ")}</p>
                      </div>
                    )}
                    {(plan.metrics?.length ?? 0) > 0 && (
                      <div>
                        <span className="font-semibold text-slate-700">Metrics:</span>
                        <p className="text-slate-500">{plan.metrics!.join(", ")}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {scriptId === plan.id && script && (
                    <div className="mt-2">
                      <details>
                        <summary className="cursor-pointer text-xs font-semibold text-emerald-600">Show Generated Script</summary>
