    ...
    <SidebarLayout>
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Research Projects</h1>
            <p className="mt-1 text-sm text-slate-500">Manage your research investigations and experiments.</p>
          </div>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
              ...
              <div className="grid gap-3">
                {projects.map((project) => (
                  <button
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-white p-4 text-left transition-all hover:border-emerald-300 hover:shadow-sm"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <h3 className="font-semibold text-slate-900">{project.name}</h3>
                        <p className="mt-1 text-sm text-slate-500 line-clamp-2">{project.description || "No description"}</p>
                      </div>
                      <svg className="mt-1 h-4 w-4 flex-shrink-0 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </div>
          ...
          <Panel title="New Project">
            {createError && (
              <div className="mb-3 rounded-md border border-red-200 bg-red-50 p-2 text-xs text-red-700">{createError}</div>
            )}
            <form className="grid gap-4" onSubmit={createProject}>
              <div>
                ...
                <input className="w-full rounded-md border border-slate-300 p-2 text-sm focus:border-emerald-500 focus:outline-none" name="name" placeholder="e.g., Literature Review on RAG" required />
              </div>
              <button
                className="rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-emerald-800 disabled:opacity-50"
                disabled={busy}
              >
