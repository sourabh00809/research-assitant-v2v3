...
async def upload_paper(project_id: str, request: Request, filename: str = "uploaded.pdf") -> UploadedPaper:
    ...
    object_record = state.object_store.put_bytes(project_id, "pdf", upload_filename, content, content_type)
...
def get_paper(project_id: str, paper_id: str) -> UploadedPaper:
    paper = state.store.get_uploaded_paper(project_id, paper_id)
    if not paper:
...
def connector_status() -> list:
    return state.orchestrator.search_service.connector_status()
